            ...
            annotations: [
                {
                    ...
                    newsletter: {
                        newsletterJid: "120363424412154238@newsletter",
                        serverMessageId: 0,
...
const generateForwardMessageContent = (message, forceForward) => {
    ...
    // >>>>>> MODIFIKASI 1: UNWRAP PESAN <<<<<<
    // Patch untuk menangani objek wrapper (seperti m.quoted dari beberapa framework bot)
    // yang menyimpan WebMessageInfo asli di dalam properti 'fakeObj'.
    // Ini memperbaiki error "no content in message" yang terjadi di awal fungsi.
    if (message && message.fakeObj) {
        message = message.fakeObj;
    }
    ...
    // >>>>>> PERBAIKAN BUG & MODIFIKASI 2: HAPUS FLAG VIEW_ONCE <<<<<<
    // Dapatkan tipe konten pesan (misal: 'imageMessage', 'videoMessage')
    // GANTI 'const' MENJADI 'let' UNTUK MEMPERBAIKI BUG DI LIBRARY!
    let key = Object.keys(content)[0];

    // Jika konten memiliki flag 'viewOnce', paksa untuk menonaktifkannya.
    // Ini memungkinkan pesan View Once untuk diteruskan tanpa batasan.
    if (content[key]?.viewOnce) {
        content[key].viewOnce = false;
    }
    // >>>>>> PERBAIKAN BUG & MODIFIKASI 2 SELESAI <<<<<<
    ...
    if (key === 'conversation') {
        ...
        key = 'extendedTextMessage'; // Sekarang baris ini tidak akan error lagi
    }
    if (score > 0) {
